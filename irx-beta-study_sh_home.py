...
#zlist=np.array([0.19, 0.9, 2.0, 3.0, 3.95, 5.96, 7, 8]) #, 8.94]) #, 9.95])
zlist = np.array([0.19])
...
def bressan_model(sfr,seds_nodust_tot):
    bress_rad_lum = []
    ...
    total_mags_nod = np.array(seds_nodust_tot, dtype=np.float64)
    q_ionis = ionising_photons(total_mags_nod, 912.0) #in s^-1
    ...
    for i, nu in enumerate(selection_freq):
        ...
        sync[i,:] = synchrotron_lum(sfr[:], nu)/1e7
   ...
   # qIR_bressan = np.log10(lir_total/3.75e12) - np.log10(lum_radio[3,:]/1e7)
    ...
    bress_rad_lum.append(lum_radio[3]/1e7)

    return np.asarray(bress_rad_lum)


def prepare_data(hdf5_data, ext_data, seds, seds_lir, index, zsnap, obsdir, sm_irx,seds_lir_rad,seds_lir_bc,seds_nodust,seds_bands):
    ...
    Lsun = 3.828 * 10**26 #W
    lir_total_ff= seds_nodust[2][1] #total IR luminosity in units of Lsun with ionising photons
    ...
    sfr = sfr[pos]
    lir_total_ff= seds_nodust[2][1] #total IR luminosity in units of Lsun with ionising photons
    lir_total_ff = lir_total_ff[pos]
    lir_dust_qir = np.array(seds_lir_rad[2][0], dtype=np.float64) ## needed to prevent precision errors
    lir_dust_qir = lir_dust_qir*Lsun
    lir_dust_qir = lir_dust_qir[pos]
    ...
    ##Test galaxy ids are the same##
    
    
    ###Compute Radio Emission with Bressan model###
    

    
    
    
    rad_lum = bressan_model(sfr/1e9,lir_total_ff)
    ...
    qir = np.log10(lir_dust_qir/3.75e12) - np.log10(rad_lum[0])
    
    print("This is length of qir")
    print(len(qir))
    print("This is length of irx")
    print(len(irx))
    print("This is length of beta")
    print(len(beta))
    ...
    return(volh, h0, irx, beta, mstar_in, zgas_in, ssfr_in, mdust_in, eta_comp, tau_birth_comp, tau_screen_comp,qir)
    
def plot_irx_beta(plt, outdir, obsdir, irx, beta, mstar, zgas_in, ssfr, mdust, eta, tau_birth, tau_screen, z, dust_model,qir):
    ...
    def plot_properties_in_irx_beta(prop, name, label, binsx=30, binsy=30, vmin=-1.5, vmax=0.5):
       ...
       for i,subplot in enumerate(subplots):
           ...
           meds_errors[i] = meds
           im = ax.hexbin(x, y, prop[inp], xscale='linear', yscale='linear', gridsize=(binsx,binsy), vmin=vmin, vmax=vmax, cmap='magma', mincnt=10, reduce_C_function=np.median)
    ...
    meds_errors = plot_properties_in_irx_beta(qir, 'qir', 'qir', binsx=30, binsy=30, vmin=0, vmax=3)
...
def main(modeldir, outdir, redshift_table, subvols, obsdir):
    ...
    file_hdf5_sed_radio = "Shark-SED-" + file_name + ".hdf5"

    fields_sed_rad = {'SED/lir_dust': ('disk','bulge_t','total'),}
    ...
    for index, snapshot in enumerate(redshift_table[zlist]):
        ...
        seds_lir_rad = common.read_photometry_data_variable_tau_screen(modeldir, snapshot, fields_sed_rad, subvols, file_hdf5_sed_radio)
        seds_lir_bc = common.read_photometry_data_variable_tau_screen(modeldir, snapshot, fields_sed_bc, subvols, file_hdf5_sed_radio)
        seds_nodust = common.read_photometry_data_variable_tau_screen(modeldir, snapshot, fields_seds_nodust, subvols, file_hdf5_sed_radio)
        seds_bands = common.read_photometry_data_variable_tau_screen(modeldir, snapshot, fields_seds_dust, subvols, file_hdf5_sed_radio)
        


        (volh, h0, irx, beta, mstar_in, zgas_in, ssfr, mdust, 
         eta, tau_birth, tau_screen,qir) = prepare_data(hdf5_data, ext_data, seds, seds_lir, index, zlist[index], obsdir, sm_irx,seds_lir_rad,seds_lir_bc,seds_nodust,seds_bands)
        
        
        
        plot_irx_beta(plt, outdir, obsdir, irx, beta, mstar_in, zgas_in, ssfr, mdust, eta, tau_birth, 
                      tau_screen, zlist[index], dust_model,qir)
