...
def prepare_data(photo_data_radio, seds_nod, ids_sed, hdf5_data, subvols, lightcone_dir,  nbands):
    ...
    writeon = True
    csv_name = 'Plots/Shark-Lightcone-radio-only-ultradeep-Bressan02_sh.csv'
  #  if(writeon == True):
   #    ascii.write(data_to_write, csv_name, names = ['dec', 'ra', 'redshift', 'log10(mstar)', 'flux_8p4GHz', 'flux_5GHz', 'flux_3GHz', 'flux_1p4GHz', 'flux_610MHz', 'flux_325MHz', 'flux_150MHz'], overwrite = True, format='csv', fast_writer=False)
    
    
    df = pd.DataFrame(data_to_write, columns = ['dec', 'ra', 'redshift', 'log10(mstar)', 'flux_8p4GHz', 'flux_5GHz', 'flux_3GHz', 'flux_1p4GHz', 'flux_610MHz', 'flux_325MHz', 'flux_150MHz'])
...
def lightcone_plt(df,areasub):
    ...
    for nu in freq_labels:
        ...
        s12_lst = []
        xlab = 's/mJy ' + str(nu)
        save_name = 'Plots/lightcone_' + str(nu) +'.pdf'
        for idx in list(range(len(mbins)-1)):
            s1 = mbins[idx]
            s2 = mbins[idx+1]
            N = len(df.loc[((df[nu] > s1)&(df[nu] < s2)),nu])
            result = np.log10(np.log10(N)/areasub*(1/(s2*1e3-s1*1e3)))
            result_lst.append(result)
            s12_lst.append(np.log10(s2-s1))
        plt.plot(s12_lst,result_lst)
        plt.xlabel(xlab)
        plt.ylabel("log(N) $Jy^{-1}$ $sr^{-1}$")
        plt.title(nu)
...
def main(lightcone_dir):
    ...
    df = prepare_data(seds_radio, seds_nod, ids_sed, hdf5_data, subvols, lightcone_dir, nbands)
    
    
    lightcone_plt(df,areasub)
...
if __name__ == '__main__':
    ...
    #running at home: python3 radio_calculation_bressan.py -S lightcone_dir -v 0 -o Plots
    #running in pawsey: python3 radio_calculation_bressan.py -S /scratch/pawsey0119/clagos/Stingray/output/medi-SURFS/Shark-TreeFixed-ReincPSO-kappa0p002/deep-optical/ -v 0-63 -o Plots