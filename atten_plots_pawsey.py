...
def prepare_data(hdf5_data, seds, seds_bands, seds_bc, index, LFs_dust, obsdir,seds_tau):
   ...
   # print(lir_selected.shape, lradio_selected.shape)
    ...
    tau_birth_bulge = seds_tau[0]
    tau_birth_disk = seds_tau[1]
    tau_screen_bulge = seds_tau[2]
    tau_screen_disk= seds_tau[3]
  

    print("THis is lir_ism_disk")
    print(lir_ism_disk)
    return(volh, h0, band_14, band_30, lir_total_W,Teff,lir_disk_w,lir_bulge_w,lir_bc_disk[0],lir_ism_disk[0],tau_birth_bulge,tau_birth_disk,tau_screen_bulge,tau_screen_disk)
    
def dataframe(hdf5_lir_lst,zlist,seds_bands_lst,seds_lir_lst,lir_total_W_lst,seds_nodust_lst,h0,Teff_lst,fields,lir_disk_w_lst,lir_bulge_w_lst,lir_bc_disk_lst,lir_ism_disk_lst,tau_birth_bulge_lst,tau_birth_disk_lst,tau_screen_bulge_lst,tau_screen_disk_lst):
    h0 = float(h0)
    ...
    for z in range(len(zlist)):
        ...
        df_temp['tau_birth_bulge'] = tau_birth_bulge_lst[z]
        df_temp['tau_birth_disk'] = tau_birth_disk[z]
        df_temp['tau_screen_bulge'] = tau_screen_bulge_lst[z]
        df_temp['tau_screen_disk'] = tau_screen_disk_lst[z]
        print(df_temp)
...
def main(model_dir, outdir, redshift_table, subvols, obsdir):
    ...
    fields_tau = {'galaxies':('tau_birth_bulge','tau_birth_disk','tau_screen_bulge','tau_screen_disk')}
    
    file_hdf5_tau = 'extinction-eagle-rr14.hdf5'
  #  fields = {'galaxies': ('mstars_disk', 'mstars_bulge','sfr_disk', 'sfr_burst','type','mgas_metals_bulge', 'mgas_metals_disk')}
    ...
    for index, snapshot in enumerate(redshift_table[zlist]):
        ...
        seds_tau = common.read_photometry_data_variable_tau_screen(model_dir, snapshot, fields_tau, subvols, file_hdf5_tau)        

        (volh, h0, band_14, band_30, lir_total_W,Teff,lir_disk_w,lir_bulge_w,lir_bc_disk,lir_ism_disk,tau_birth_bulge,tau_birth_disk,tau_screen_bulge,tau_screen_disk) = prepare_data(hdf5_data, seds_lir, seds_bands, seds_lir_bc, index, LFs_dust, obsdir,seds_tau)
        ...
        tau_birth_bulge_lst.append(tau_birth_bulge)
        tau_birth_disk_lst.append(tau_birth_disk)
        tau_screen_bulge_lst.append(tau_screen_bulge)
        tau_screen_disk_lst.append(tau_screen_disk)
    ...
    df = dataframe(hdf5_lir_lst,zlist,seds_bands_lst,seds_lir_lst,lir_total_W_lst,seds_nodust_lst,h0,Teff_lst,fields,lir_disk_w_lst,lir_bulge_w_lst,lir_bc_disk_lst,lir_ism_disk_lst,tau_birth_bulge_lst,tau_birth_disk_lst,tau_screen_bulge_lst,tau_screen_disk_lst)
    print('df2 takes:')
